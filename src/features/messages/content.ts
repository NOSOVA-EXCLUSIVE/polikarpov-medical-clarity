...
export function getThreadRulesText(productCode: ProductCode) {
  switch (productCode) {
    case "SECOND_OPINION":
      return "Это окно коротких уточнений после разбора. Здесь можно задать до 3 сообщений в течение 72 часов. Новый кейс, повторный полный разбор и отправка больших MRI, видео или архивов без решения врача здесь не проводятся.";
    case "MEDICAL_ROUTE":
    ...
    case "RECOVERY_4_WEEKS":
      return "Центр сообщений открыт на срок пакета сопровождения. Используйте его для рабочих вопросов по динамике состояния и самочувствию в рамках согласованного формата.";
    case "PERSONAL_SUPPORT":
      return "Центр сообщений открыт на срок активного сопровождения. Используйте его для текущих вопросов по кейсу и организационных уточнений в рамках согласованного формата.";
    default:
...
export function getOpeningSystemMessage(productCode: ProductCode) {
  switch (productCode) {
    case "SECOND_OPINION":
      return "Окно уточнений открыто. Здесь можно задать короткие вопросы по итогам разбора в течение 72 часов.";
    case "MEDICAL_ROUTE":
      return "Окно уточнений по маршруту открыто. Здесь можно уточнить шаги и последовательность действий в течение 7 дней.";
    case "RECOVERY_4_WEEKS":
      return "Формат сопровождения активирован. Этот центр сообщений используйте для текущих вопросов по динамике состояния в течение пакета.";
    case "PERSONAL_SUPPORT":
      return "Индивидуальное сопровождение активировано. Этот центр сообщений используйте для текущих вопросов по кейсу в течение активного пакета.";
    default:
